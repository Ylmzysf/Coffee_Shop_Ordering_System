...
router.post("/login", async (req, res) => {
    ...
    try {
        ...
        const query = 'SELECT * FROM kullaniciler WHERE kullanici_adi = ?';
        ...
        if (results.length > 0) {
            // Debug log
            console.log('Stored hashed password:', results[0].sifre);
            console.log('Attempting to compare with provided password');
            
            const match = await bcrypt.compare(password, results[0].sifre);
...
// Kullanıcı ekleme sayfası route'u
router.get('/kullanici-ekle', checkAuth, (req, res) => {
    res.render('kullanici-ekle'); // Bu view'i oluşturmanız gerekecek
});

// Yeni kullanıcı ekleme route'u
router.post('/kullanici-ekle', checkAuth, async (req, res) => {
    const { username, password } = req.body;
    
    try {
        // Kullanıcı adı kontrolü
        const [existingUser] = await mysql.execute(
            'SELECT * FROM kullaniciler WHERE kullanici_adi = ?', 
            [username]
        );

        if (existingUser.length > 0) {
            return res.render('kullanici-ekle', {
        ...
        res.redirect('/kullanici-listesi'); // veya başka bir sayfaya yönlendirin
    } catch (error) {
